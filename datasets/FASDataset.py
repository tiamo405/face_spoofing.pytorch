...
class FASDataset(Dataset):
    ...
    def __init__(self, root_dir, csv_file, depth_map_size, transform, smoothing):
        ...
        for idfolder in os.listdir(self.root_dir) :
            for label in os.listdir(os.path.join(self.root_dir, idfolder)) :
                for fname in os.listdir(os.path.join(self.root_dir, idfolder, label)) :
                    if '.jpg' in fname :
                        path_img.append(os.path.join(self.root_dir, idfolder, label, fname))
                        label_img.append(1 if label == "live" else 0)
